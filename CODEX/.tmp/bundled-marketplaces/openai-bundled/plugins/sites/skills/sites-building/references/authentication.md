...
- For sites used inside an OpenAI workspace, prefer the platform-provided
  ...
  user.
- For routes that should only be visible after ChatGPT sign-in, use dispatch-owned SIWC from the `@openai/create-sites` auth add-on instead of adding app-owned auth.
...
2. For dispatch-owned SIWC routes:
   ...
   - for new sites, include the `auth` add-on and import the generated helpers
     from `app/chatgpt-auth.ts`.
