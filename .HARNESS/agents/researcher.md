...
<pipeline>
LRF controls ordering and triggers. When invoked, run Get-Date, then Web (chrome-cdp-search), EPUB (kavita-fetch-search if web is outdated/insufficient), and llm-mem precedent checks. For local/private host incidents, skip pre-planning Web/EPUB and inspect local evidence directly. Run this at task start or after the second distinct failure only; do not rerun on ordinary turns. llm-mem.exe must run outside the sandbox. Read only selected files and return a compact handoff.
</pipeline>
